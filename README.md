...
let oauth = BattleNetOAuth(scope: [.d3, .sc2, .wow, .openID], scheme: "BattleNetApp", redirectUrl: "https://redirect.to/BattleNetApp", providerContext: AuthenticationContext())
let battleNetAPI = BattleNetAPI(credentials: current.credentials, oauth: current.oauth)
```
...
Others functions, known as Community or Profile APIs, require a player to first authorize your app by signing into their BattleNet account.

``` swift
try await battleNetAPI.authenticationManager.getUserAccessToken()
```

If the player authorizes your app, you can then use the same client to call Community and Profile APIs.
...
If you plan to call Community or Profile APIs, you'll need to support the user OAuth process. This requires a redirect url where the authorization code will be sent. iOS and Mac apps can be opened by a url with a custom url scheme, but unfortunately Blizzard does not allow url schemes other than https. A possible solution is discussed in the OAuth section below.
...
```swift
let battleNetAPI = BattleNetAPI(credentials: credentials, oauth: oauth, session: .shared, region: .us, locale: .en_US)
```

## Handling Results

Each API function supports either async/await or a `completion` handler. The response is returned as the `Data` type, and any error will typically be of type `HTTPError`, an enum within the BattleNetAPI framework.
...
The framework attempts to provide structs for each response, but Blizzard has not released documentation on any schema, and therefore accuracy cannot be guaranteed. These functions are nested within a `decoded` struct within the web service class.

```swift
let act1Data = try await battleNetAPI.d3.getAct(id: 1) // returns type Data
let act1Struct = try await battleNetAPI.d3.decoded.getAct(id: 1) // returns type Act
```

To reiterate, the accuracy of the objects cannot be guaranteed, so if there is a JSON parsing error, it's recommended you fall back on the functions that return `Data` instead.


## Access Tokens

Calling each API requires either a clientAccessToken (for Game Data APIs) or a userAccessToken (for Community and Profile APIs). The BattleNetAPI client will manage both automatically once they're initially retrieved, but it's recommended you store these tokens to reinitialize the client each time the app launches. 

Assign a `battleNetAPI.delegate` to be notified when the tokens change. Once you've saved the tokens, you can include them in the BattleNetCredentials when you initialize the BattleNetAPI client.

```swift
let credentials = BattleNetCredentials(clientID: "CLIENT_ID",
...
battleNetAPI.delegate = BattleNetAPIDelegate()

// Save tokens for subsequent launches
class BattleNetAPIDelegate: BattleNetAuthDelegate {
...
The tokens are validated when the BattleNetAPI instance is created, and if invalid or expired, will call the delegate methods with `nil` values for the token.
...
* **WebServices Group**: Contains all the web service functions divided by game as well as for authentication and user data. Although these can be used directly, it's strongly recommended you access these API functions from an instance of the BattleNetAPI class instead.
    * **+Decoded**: The API functions that attempt to map to structs found in the Model
* **Model Group**: 
	* **Auth Subgroup**: Includes the BattleNetCredentials and BattleNetOAuth classes you'll need to initialize the API client.
	* **Object Subgroup**: This contains the structs that map to their web service responses. They are divided by game, as well as for authentication and user data.
		* Note that the structs may not be 100% accurate as Blizzard does not provide complete documentation for all possible values a web service might return. The structs were constructed based off a few test responses.
...
The BattleNetAPI class is responsible for managing the tokens needed to call the web services. It edits the BattleNetAPI Credentials with the clientAccessToken and userAccessToken once they are initially retrieved, but it doesn't store them persistently. See the Access Tokens section above for more details about this process.

The `battleNetAPI.authenticationManager` is especially helpful with asking the player to sign into their BattleNet account and authorizing your app. However, this requires you to configure your app to be opened by a url, and to handle the OAuth redirect outside the app.
...
`BattleNetOAuth(scope:scheme:redirectUrl:providerContext:)`
...
The `providerContext` is actually not so complicated. It's any class that's essentially telling Apple how to present the sign in to the user.
...
```swift
let oauth = BattleNetOAuth(scope: [.d3, .sc2, .wow, .openID], scheme: "BattleNetAPI", redirectUrl: "https://redirect.to/BattleNetAPI", providerContext: AuthenticationContext())
```

### User Authentication

You call `battleNetAPI.authenticationManager.getUserAccessToken()` to begin the user authentication process. A web view will be presented with the BattleNet sign in, and the user will see which games you're asking them to permit your app access to.
